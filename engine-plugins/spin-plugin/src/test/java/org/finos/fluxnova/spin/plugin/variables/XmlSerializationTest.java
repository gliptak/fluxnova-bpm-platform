...
public class XmlSerializationTest extends PluggableProcessEngineTestCase {
  ...
  public void testOverloadedAppendMethod() {
    ...
    deployment(Bpmn.createExecutableProcess("spin-xml-issue")
        .startEvent()
        .serviceTask()
          .fluxnovaExpression("${XML(\"<result/>\").append(xmlInput.xPath(\"//cosigner/*\").elementList()).toString()}")
          .fluxnovaResultVariable("output")
        .userTask()
        .endEvent()
        .done());

    Reader xmlInput = SpinIoUtil.classpathResourceAsReader("org/finos/fluxnova/spin/plugin/XmlSerializationTest-input.xml");
    String expectedOutput = SpinIoUtil.fileAsString("org/finos/fluxnova/spin/plugin/XmlSerializationTest-output.xml");
