...
public class TaskMetricsTest {
  ...
  protected static final BpmnModelInstance USER_TASK_PROCESS = Bpmn.createExecutableProcess(PROCESS_KEY)
      ...
      .startEvent()
      .userTask("task").fluxnovaAssignee("kermit")
      .endEvent()
      .done();
  ...
  public void shouldCreateTaskMetricWithAssignmentByTaskListener() {
    ...
    testRule.deploy(Bpmn.createExecutableProcess(PROCESS_KEY)
        .fluxnovaHistoryTimeToLive(180)
        .startEvent()
        .userTask("task").fluxnovaTaskListenerClass("create", AssignmentTaskListener.class)
        .endEvent()
        .done());
    // when
    runtimeService.startProcessInstanceByKey(PROCESS_KEY);
  ...
  @Test
  public void shouldCreateTaskMetricWithAssignmentByFluxnovaAssigneeExtension() {
    // given
    testRule.deploy(USER_TASK_PROCESS);
    // when
    runtimeService.startProcessInstanceByKey(PROCESS_KEY);
  ...
  public void shouldCreateTaskMetricsWithMultipleAssignments() {
    ...
    testRule.deploy(Bpmn.createExecutableProcess(PROCESS_KEY)
        ...
        .startEvent()
        .userTask("task")
          .fluxnovaAssignee("kermit")
          .fluxnovaTaskListenerClass("create", AssignmentTaskListener.class)
        .endEvent()
        .done());
    // when
    runtimeService.startProcessInstanceByKey(PROCESS_KEY);
