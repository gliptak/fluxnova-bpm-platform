...
import org.finos.fluxnova.bpm.engine.test.bpmn.async.FailingDelegate;
...
public class RemovalTimeStrategyStartTest extends AbstractRemovalTimeTest {
  ...
  public void shouldResolveExternalTaskLog() {
    ...
    testRule.deploy(Bpmn.createExecutableProcess("calledProcess")
      .startEvent()
        .serviceTask().fluxnovaExternalTask("anExternalTaskTopic")
      .endEvent().done());
  ...
  public void shouldResolveByteArray_ExternalTaskLog() {
    ...
    testRule.deploy(Bpmn.createExecutableProcess("calledProcess")
      .startEvent()
        .serviceTask().fluxnovaExternalTask("aTopicName")
      .endEvent().done());
  ...
  public void shouldResolveByteArray_DecisionInput() {
    ...
    testRule.deploy(Bpmn.createExecutableProcess(CALLING_PROCESS_KEY)
      .fluxnovaHistoryTimeToLive(5)
      .startEvent()
        .businessRuleTask().fluxnovaDecisionRef("testDecision")
      .endEvent().done());
  ...
  @Test
  @Deployment( resources = {
    "org/finos/fluxnova/bpm/engine/test/api/history/removaltime/HistoricRootProcessInstanceTest.shouldResolveByteArray_DecisionOutputLiteralExpression.dmn"
  })
