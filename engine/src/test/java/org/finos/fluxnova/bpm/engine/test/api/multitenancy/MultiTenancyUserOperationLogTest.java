...
public class MultiTenancyUserOperationLogTest {
  ...
  protected static final BpmnModelInstance MODEL_JOB = Bpmn.createExecutableProcess(PROCESS_NAME)
      .startEvent().userTask(TASK_ID).fluxnovaAsyncBefore().done();
  ...
  public void shouldLogDeployOperationWithTenant() {
    // given
    identityService.setAuthentication(USER_ID, null, Arrays.asList(TENANT_ONE));
    String deploymentId = testRule.deployForTenant(TENANT_ONE, "org/finos/fluxnova/bpm/engine/test/api/externaltask/externalTaskPriorityExpression.bpmn20.xml").getId();
  ...
  public void shouldLogExternalTaskOperationWithTenant() {
    // given
    testRule.deployForTenant(TENANT_ONE, "org/finos/fluxnova/bpm/engine/test/api/externaltask/externalTaskPriorityExpression.bpmn20.xml");
    identityService.setAuthentication(USER_ID, null, Arrays.asList(TENANT_ONE));
  ...
  public void shouldLogDecisionDefinitionOperationWithTenant() {
    // given
    testRule.deployForTenant(TENANT_ONE, "org/finos/fluxnova/bpm/engine/test/api/dmn/Example.dmn");
    identityService.setAuthentication(USER_ID, null, Arrays.asList(TENANT_ONE));
  ...
  public void shouldLogCaseDefinitionOperationWithTenant() {
    // given
    testRule.deployForTenant(TENANT_ONE, "org/finos/fluxnova/bpm/engine/test/api/cmmn/oneTaskCase.cmmn");
    identityService.setAuthentication(USER_ID, null, Arrays.asList(TENANT_ONE));
