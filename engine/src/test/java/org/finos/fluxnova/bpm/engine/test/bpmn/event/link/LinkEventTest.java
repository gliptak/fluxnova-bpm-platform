 ...
 */
package org.finos.fluxnova.bpm.engine.test.bpmn.event.link;
...
public class LinkEventTest extends PluggableProcessEngineTest {
  ...
  public void testInvalidEventLinkMultipleTargets() {
    try {
      repositoryService.createDeployment().addClasspathResource("org/finos/fluxnova/bpm/engine/test/bpmn/event/link/LinkEventTest.testInvalidEventLinkMultipleTargets.bpmn20.xml").deploy();
  ...
  public void testCatchLinkEventAfterEventBasedGatewayNotAllowed() {
    try {
      repositoryService.createDeployment().addClasspathResource("org/finos/fluxnova/bpm/engine/test/bpmn/event/link/LinkEventTest.testCatchLinkEventAfterEventBasedGatewayNotAllowed.bpmn20.xml").deploy();
      fail("process should not deploy because it contains multiple event link targets which is invalid in the BPMN 2.0 spec");
    }
    catch (ParseException e) {
