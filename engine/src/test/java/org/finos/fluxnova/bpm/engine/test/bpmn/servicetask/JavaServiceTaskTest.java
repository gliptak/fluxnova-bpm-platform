...
import org.finos.fluxnova.bpm.engine.ClassLoadingException;
...
import org.finos.fluxnova.bpm.engine.test.bpmn.servicetask.util.GenderBean;
...
public class JavaServiceTaskTest extends PluggableProcessEngineTest {
  ...
  public void testUnexistingClassDelegation() {
    try {
      ...
      assertTrue(e.getMessage().contains("Exception while instantiating class 'org.finos.fluxnova.bpm.engine.test..BogusClass'"));
  ...
  public void testIllegalUseOfResultVariableName() {
    try {
      repositoryService.createDeployment().addClasspathResource("org/finos/fluxnova/bpm/engine/test/bpmn/servicetask/JavaServiceTaskTest.testIllegalUseOfResultVariableName.bpmn20.xml").deploy();
      fail();
    } catch (ProcessEngineException e) {
