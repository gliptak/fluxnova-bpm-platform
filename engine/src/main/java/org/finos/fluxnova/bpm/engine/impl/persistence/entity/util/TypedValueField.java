 ...
 */
package org.finos.fluxnova.bpm.engine.impl.persistence.entity.util;
...
import org.finos.fluxnova.bpm.engine.impl.variable.serializer.ValueFieldsImpl;
...
public class TypedValueField implements DbEntityLifecycleAware, CommandContextListener {
  ...
  public TypedValue setValue(TypedValue value) {
    ...
    if(value instanceof UntypedValueImpl impl) {
      // type has been detected
      value = serializer.convertToTypedValue(impl);
    }
    ...
    if (Context.getCurrentProcessApplication() != null) {
      ...
      try {
        ...
        if (rawPa instanceof AbstractProcessApplication application) {
          return application.getVariableSerializers();
        }
        else {
          return null;
        }
