...
import org.finos.fluxnova.bpm.engine.test.assertions.bpmn.AbstractProcessAssert;
import org.finos.fluxnova.bpm.engine.test.assertions.bpmn.TaskAssert;
...
public abstract class AbstractCaseAssert<S extends AbstractCaseAssert<S, A>, A extends CaseExecution> extends AbstractProcessAssert<S, A> {
   ...
   * @throws  org.finos.fluxnova.bpm.engine.ProcessEngineException in case more than
  ...
  protected String toString(A caseExecution) {
    if (caseExecution != null) {
      return !actual.getCaseInstanceId().equals(actual.getId()) ? "%s {id='%s', activityId='%s' }".formatted(
