 ...
 */
package org.finos.fluxnova.spin.impl.json.jackson;
...
import org.finos.fluxnova.spin.impl.SpinListImpl;
import org.finos.fluxnova.spin.impl.json.jackson.format.JacksonJsonDataFormat;
import org.finos.fluxnova.spin.impl.json.jackson.format.JacksonJsonDataFormatMapper;
import org.finos.fluxnova.spin.impl.json.jackson.query.JacksonJsonPathQuery;
...
import org.finos.fluxnova.spin.json.SpinJsonPathQuery;
...
public class JacksonJsonNode extends SpinJsonNode {
  ...
  public Integer indexOf(Object searchObject) {
    ...
    if(res == -1){
      throw LOG.unableToFindProperty(node.toString());
    }
    return res;
  }
  ...
  public SpinList<SpinJsonNode> elements() {
    if(jsonNode.isArray()) {
      Iterator<JsonNode> iterator = jsonNode.values().iterator();
  ...
  public List<String> fieldNames() {
    if(jsonNode.isContainer()) {
      Iterator<String> iterator = jsonNode.propertyNames().iterator();
