...
public class ProcessTest extends BpmnModelElementInstanceTest {
  ...
  public void testFluxnovaJobPriority() {
    ...
    assertThat(process.getFluxnovaJobPriority()).isNull();

    process.setFluxnovaJobPriority("15");

    assertThat(process.getFluxnovaJobPriority()).isEqualTo("15");
  }

  @Test
  public void testFluxnovaTaskPriority() {
    //given
    Process proc = modelInstance.newInstance(Process.class);
    assertThat(proc.getFluxnovaTaskPriority()).isNull();
    //when
    proc.setFluxnovaTaskPriority(BpmnTestConstants.TEST_PROCESS_TASK_PRIORITY);
    //then
    assertThat(proc.getFluxnovaTaskPriority()).isEqualTo(BpmnTestConstants.TEST_PROCESS_TASK_PRIORITY);
  }
  ...
  public void testFluxnovaHistoryTimeToLive() {
    //given
    Process proc = modelInstance.newInstance(Process.class);
    assertThat(proc.getFluxnovaHistoryTimeToLive()).isNull();
    //when
    proc.setFluxnovaHistoryTimeToLive(BpmnTestConstants.TEST_HISTORY_TIME_TO_LIVE);
    //then
    assertThat(proc.getFluxnovaHistoryTimeToLive()).isEqualTo(BpmnTestConstants.TEST_HISTORY_TIME_TO_LIVE);
  }
}
