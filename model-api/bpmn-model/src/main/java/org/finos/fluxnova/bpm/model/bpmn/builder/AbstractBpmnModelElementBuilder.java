...
public abstract class AbstractBpmnModelElementBuilder<B extends AbstractBpmnModelElementBuilder<B, E>, E extends BpmnModelElementInstance> {
  ...
  public SubProcessBuilder subProcessDone() {
    ...
    if (lastSubProcess != null && lastSubProcess instanceof SubProcess process) {
  ...
  public AbstractThrowEventBuilder throwEventDefinitionDone() {
    ...
    if (lastEvent != null && lastEvent instanceof IntermediateThrowEvent event1) {
      return new IntermediateThrowEventBuilder(modelInstance, event1);
    } else if (lastEvent != null && lastEvent instanceof EndEvent event) {
      return new EndEventBuilder(modelInstance, event);
    }
    else {
