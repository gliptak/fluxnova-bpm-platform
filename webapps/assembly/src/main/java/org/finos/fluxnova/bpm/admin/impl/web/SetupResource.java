 ...
 */
package org.finos.fluxnova.bpm.admin.impl.web;
...
import org.finos.fluxnova.bpm.engine.rest.impl.UserRestServiceImpl;
...
public class SetupResource {
  ...
  protected void createInitialUserInternal(String processEngineName, UserDto user, ProcessEngine processEngine) {
    ...
    ensureFluxnovaAdminGroupExists(processEngine);
  ...
  protected void ensureFluxnovaAdminGroupExists(ProcessEngine processEngine) {
    ...
    if(identityService.createGroupQuery().groupId(Groups.CAMUNDA_ADMIN).count() == 0) {
      Group fluxnovaAdminGroup = identityService.newGroup(Groups.CAMUNDA_ADMIN);
      fluxnovaAdminGroup.setName("Fluxnova BPM Administrators");
      fluxnovaAdminGroup.setType(Groups.GROUP_TYPE_SYSTEM);
      identityService.saveGroup(fluxnovaAdminGroup);
    }
